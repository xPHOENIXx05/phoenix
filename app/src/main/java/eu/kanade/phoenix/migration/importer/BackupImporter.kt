...
class BackupImporter(
    ...
    fun importBackup(uri: Uri): PhoenixBackup {
        ...
        android.util.Log.d(
            "PhoenixMigration",
            "Decoded manga count = ${backup.backupManga.size}"
        )

        backup.backupManga.firstOrNull()?.chapters?.take(5)?.forEach {
            android.util.Log.d(
                "PhoenixMigration",
                "Decoded -> ${it.name} read=${it.read} bookmark=${it.bookmark} lastPage=${it.lastPageRead}"
            )
        }
